...
function normalizeEntry(raw) {
  ...
  const duration = typeof payload.duration === 'number' ? payload.duration : raw.duration;
  ...
  return {
    ...
    module,
    page: payload.page || raw.page || payload.component || raw.component || 'unknown',
...
function analyzeEntries(entries) {
  ...
  for (const entry of entries) {
    ...
    const routeEntry =
      routeMap.get(route) || { route, count: 0, total: 0, max: 0, functions: new Set() };
    routeEntry.count += 1;
    ...
    if (
      ...
      const repeatEntry =
        repeatCallMap.get(repeatKey) || { name: entry.name, file: entry.file, count: 0, totalDuration: 0 };
      repeatEntry.count += 1;
...
function buildSpeedscope(entries, profileName = 'RN Function Perf') {
  ...
  const minTs = usable.reduce((m, e) => Math.min(m, e.ts - e.duration), Infinity);
...
function writeAnalysisOutputs({ entries, reportPath, speedscopePath, markdownPath }) {
  ...
  if (markdownPath) {
    ...
    const knownRoutes = (analysis.routes || []).filter((r) => r.route !== 'unknown');
